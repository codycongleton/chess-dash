...
2c. **Weekly / monthly wins vs losses** — the same chart bucketed by Mon-start week, then calendar month, directly below. All three come from `renderDiverge(period)`; per-period settings (DOM id prefix, bucket start, bar width, tooltip title) live in `DIVERGE_PERIODS`. Each chart computes its own `peak`, so bar heights aren't comparable between the daily, weekly and monthly charts.

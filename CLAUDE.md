...
2c. **Weekly wins vs losses** — the same chart bucketed by Mon-start week, directly below. Both come from `renderDiverge(period)`; per-period settings (DOM id prefix, bucket start, bar width, tooltip title) live in `DIVERGE_PERIODS`. Its `peak` is computed separately, so weekly bar heights aren't comparable to daily ones.
